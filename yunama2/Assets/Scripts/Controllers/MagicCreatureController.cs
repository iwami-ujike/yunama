...
public class MagicCreatureController : MonoBehaviour
{
    // 0→上 1→左 2→右 3→下
    ...
    void exchangeEnergyOrMagic(bool isMagic, bool isDrain) {
        int[] possibleDirections = new int[4];
        int possibleDirectionsIdx = 0;
        int[] position = currentPosition();
        ...
        for(int i=0; i<4; i++){
            ...
            if(block != null) {
                ...
                if (blockController.magicAmount > 0) {
                    possibleDirections[possibleDirectionsIdx] = i;
                    possibleDirectionsIdx++;
                }
            }
    ...
    public void setCreatureStatus(string newName, int carryingMagicAmount) {
        ...
        if (isCarryType) {
           carryingAmount = carryingMagicAmount;
           carrying = true; 
        } 
    }
}
